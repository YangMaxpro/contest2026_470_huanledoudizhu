...
| Wi-Fi 6 | 网络接口 `xiaopai`（v18 起） | v17 已验证关联、DHCP、ICMP 和 DNS；接口存在不代表已联网 |
...
网络接口名不是 MAC 地址或芯片 ID。v18 通过
`CONFIG_BK7258_WIFI_IFNAME="xiaopai"` 命名，驱动和能力探测共用该配置；
v17 及更早的测试固件仍使用 `wlan0`。网络命令示例：
...
`renew` 和 `httpcheck` 需先完成热点关联。`httpcheck [http://host/path]`
使用 NuttX HTTP 客户端，默认访问 `http://example.com/`，只统计状态码和
响应体字节数。响应体上限 32 KiB，不保存网页、不跟随重定向、不支持 TLS。
套接字处理有 30 秒期限；同步 DNS 查询仍受系统解析器超时设置控制。
`ask` 尚未接入云端，`demo` 是本地状态机演示，不是真实语音唤醒或问答。
